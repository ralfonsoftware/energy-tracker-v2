...
- [x] **Task 1: Extend `CompareTariff` with the bonus-included figure and both verdict flags** (AC: #1, #2)
  - [x] `src/EnergyTracker.Application/CompareTariff.cs`: after the existing `bonusNormalizedAnnualSavings` computation, add:
    ```csharp
    ...
    ```
  - [x] Add `CandidateAnnualCostBonusIncluded`, `BonusIncludedAnnualSavings`, `IsBonusIncludedWorthSwitching`, `IsBonusNormalizedWorthSwitching` to the `TariffComparisonResult` record (append after the existing `BonusNormalizedAnnualSavings` field — all call sites use named arguments, so field order is safe to extend) and pass them through the final `return new TariffComparisonResult(...)`.
  - [x] Verdict computation stays in the Application layer, inline, next to the rest of `CompareTariff`'s arithmetic — **do not** add a new `Domain.Calculations` helper for this. Unlike `BonusDecayNormalizer` (genuinely shared with Pattern Detective, AD-5), nothing else in the product computes an "is this a positive number" verdict, so there's no reuse case for a shared module. This does mirror the *design instinct* behind `GetCurrentStatus` computing its `Status` enum server-side rather than leaving threshold logic to the frontend (see Dev Notes) — but doesn't rise to an AD-level shared-module rule.

- [x] **Task 2: Extend the API response** (AC: #1, #2)
  - [x] `src/EnergyTracker.Api/Endpoints/TariffEndpoints.cs`: add the same four fields (`CandidateAnnualCostBonusIncluded`, `BonusIncludedAnnualSavings`, `IsBonusIncludedWorthSwitching`, `IsBonusNormalizedWorthSwitching`) to `TariffComparisonResponse`, and pass them through in `ToComparisonResponse`. No request-shape change — `CompareTariffRequest` is untouched, this is a response-only extension.
  - [x] `web/src/lib/tariff-api.ts`: add the same four fields (camelCase: `candidateAnnualCostBonusIncluded`, `bonusIncludedAnnualSavings`, `isBonusIncludedWorthSwitching`, `isBonusNormalizedWorthSwitching`) to the `TariffComparisonDto` interface — this is the type Task 5's frontend rendering reads from; forgetting this update makes the new fields `any`/untyped at the call site with no compile-time safety.

- [x] **Task 3: Add the dedicated attractiveness-signal color tokens** (AC: #3)
  - [x] `web/src/index.css`: add to the `@theme inline` block (mirroring the existing `--color-status-*` mappings at lines 51-59):
    ```css
    ...
    ```
  - [x] Add the raw variables to **both** the `:root` block and the `.dark` block. `mockups/key-tariff-radar.html` (lines 100-118) is **dark-mode only** — its hex values are verbatim-correct for `.dark`:
    ```css
    /* .dark block */
  ...
  - [x] Do **not** reuse `--status-below-baseline`/`--status-trending`/`--destructive` for any of this — `colors.md`'s own reasoning (lines 56-77 in the mockup's header comment) explicitly rejects both, and this rejection is a hard constraint, not a stylistic preference.

- [x] **Task 4: Frontend — current-vs-candidate tariff summary panels** (AC: #4)
  - [x] In `web/src/components/tariff/tariff-comparison-form.tsx`, once a non-null result exists, render two stacked `GlassCard` panels above the signal card — mirrors `mockups/key-tariff-radar.html`'s `.card` structure (lines 276-293) but using this codebase's own established label/value row pattern (`status-detail-dialog.tsx` lines 113-135: `flex items-baseline justify-between gap-4` wrapper, `text-muted-foreground text-sm` label, `text-sm font-semibold tabular-nums` value) — **do not** invent a new row-layout convention when one is already established and does the same job.
  - [x] Current tariff panel: base fee (`result.currentMonthlyBaseFee`), price/kWh (`result.currentPricePerKwh`), both formatted in `result.currency`. Candidate panel: candidate base fee, candidate price/kWh, candidate switching bonus (omit the switching-bonus row entirely when it's `0` — nothing was entered, showing "€0.00" as a row implies a deliberate zero-bonus candidate rather than "not entered").
  - [x] No tariff *names* anywhere — unlike the mockup's "Stadtwerke Flex 12" / "GrünStrom Wechselbonus 12" headings, `Tariff` (`src/EnergyTracker.Domain/Tariff.cs`) has no name/label field and this story does not add one. Use generic headings (e.g. "Your current tariff" / "Candidate tariff") instead of a per-entry name.
  - [x] Reuse the same `moneyFormat`/`kwhFormat` `Intl.NumberFormat` instances the component already constructs (lines 66-70 of the current file) — don't create parallel formatter instances for the summary panels.

- [x] **Task 5: Frontend — the two-way signal card** (AC: #1, #2, #3)
  - [x] Replace the current single-line result block (`web/src/components/tariff/tariff-comparison-form.tsx` lines 141-155 — the `result.bonusNormalizedAnnualSavings >= 0 ? savingsPositive : savingsNegative` block) with a signal card rendering **both** rows together, always, never toggled. This is a hard replacement, not an addition — the old single-figure sentence is exactly what Story 5.3 was scoped to supersede (5.2's own Scope Reality Check: "Story 5.3's Two-Way Attractiveness Signal... [is] explicitly excluded" from 5.2).
  - [x] Per row (bonus-included, bonus-normalized), drive color/badge from that row's **own** verdict flag (`result.isBonusIncludedWorthSwitching` / `result.isBonusNormalizedWorthSwitching`) independently — never assume a fixed worth/not-worth row order (`DESIGN/components.md`'s explicit instruction: "colors always follow the verdict, not fixed row order"). Structurally mirror `status-card.tsx`'s `DOT_CLASS`/`BADGE_CLASS` lookup-table pattern (lines 48-60): a `Record<boolean, string>`-shaped (or simple ternary) class map keyed on the verdict boolean, not inline conditional class strings scattered through JSX.
  - [x] Badge: a `Badge` component (`@/components/ui/badge`), `variant="outline"` override exactly like `status-card.tsx`'s own badge usage, filled with the raw `bg-attractiveness-worth-it`/`bg-attractiveness-not-worth-it` token (not the `-bg` tint — the badge itself is an opaque solid pill per the mockup, `text-attractiveness-*-badge-text` for its label color) and the plain-language verdict word (`t('tariff.compare.signal.worthBadge')` / `notWorthBadge`) — **never color alone**, exactly as `colors.md`/`components.md` require.
  - [x] Row background uses the translucent `-bg` token (`bg-attractiveness-worth-it-bg` / `bg-attractiveness-not-worth-it-bg`); the amount figure uses the raw token for the worth-it case (`text-attractiveness-worth-it`, already clears AA per the mockup's own comment) and the dedicated `text-attractiveness-not-worth-it-text` token for the not-worth-it case (the raw `--attractiveness-not-worth-it` measured only 3.50:1 at that text weight per the mockup's own verification comment — do not use the raw token there); the supporting detail sentence under each badge uses `text-attractiveness-signal-supporting-text` in both rows.
  - [x] Detail-sentence copy: templated, not the mockup's literal vendor-name sentences (no tariff names exist in this data model — see Task 4). Bonus-included row: state the switching-bonus amount and that it's credited once (skip this sentence entirely when the switching bonus is `0` — there's nothing to explain). Bonus-normalized row: state that once the bonus is gone, the candidate's ongoing rate costs more/less than the current tariff at the household's actual pace — reuse the existing `paceFootnote` figure/copy discipline (`result.annualPaceKwh`) already established in this file rather than restating pace differently per row.
  - [x] `IsLowConfidence` footnote (currently rendered once, below the single savings line) stays — render it once, below both signal rows, not duplicated per row.
...
- [x] **Task 6: i18n — both locales** (AC: #1, #3, #4)
  - [x] Add `tariff.compare.summary.*` (panel headings, base-fee/price/switching-bonus field labels — reuse `tariff.form.monthlyBaseFeeLabel`/`pricePerKwhLabel`/`tariff.compare.candidateSwitchingBonusLabel` where the exact same concept already has a key rather than adding near-duplicates) to `web/src/locales/en-US/translation.json` and `de-DE/translation.json`.
  - [x] Add `tariff.compare.signal.*`: `heading` ("Is it worth switching?"), `worthBadge` ("Worth switching"), `notWorthBadge` ("Not worth it"), bonus-included row label/detail/positive/negative sentence keys, bonus-normalized row label/detail/positive/negative sentence keys.
...
- [x] [Review][Patch] Sub-cent rounding mismatch between the worth-switching verdict and the displayed amount — the backend decided `IsBonusIncludedWorthSwitching`/`IsBonusNormalizedWorthSwitching` off an exact `> 0m` decimal comparison, while the frontend displays the corresponding amount rounded to 2 decimals. A savings value like €0.003 would read "Worth switching" next to "Save about 0.00 EUR this year" — visually indistinguishable from a tie, which AC #2 says should read "not worth it." **Resolved with Ralf:** round to 2 decimals (`MidpointRounding.AwayFromZero`, matching the frontend's own money-display rounding) before applying the tie-break, so the verdict and the displayed figure can never visually disagree. Fixed — added `decimal.Round(..., 2, MidpointRounding.AwayFromZero)` to both tie-breaks, plus a regression test. [src/EnergyTracker.Application/CompareTariff.cs:76-81], [web/src/components/tariff/tariff-comparison-form.tsx:127]
- [x] [Review][Patch] Bonus-normalized signal row has no detail sentence (Task 5 requirement) — Task 5 explicitly specifies a detail sentence for the bonus-normalized row ("state that once the bonus is gone, the candidate's ongoing rate costs more/less... reuse the existing paceFootnote figure/copy discipline"), but the second `SignalRow` is never passed a `detail` prop, and no matching i18n key exists in either locale file. Fixed — added `signal.bonusNormalizedDetail` to both locales and wired it to the row. [web/src/components/tariff/tariff-comparison-form.tsx:262-274]
- [x] [Review][Patch] "Even with the bonus" headline shown when no switching bonus was entered — `signal.bonusIncludedNegative` (en-US and de-DE) renders whenever the bonus-included row is not worth switching, with no guard on `candidateSwitchingBonus > 0`, so a candidate compared with a blank/0 bonus still shows "About X more this year, even with the bonus" — factually wrong, since no bonus was applied. The sibling `detail` sentence is correctly suppressed at bonus=0 but the parent headline text is not. Fixed — added a `bonusIncludedNegativeNoBonus` variant (bonus-agnostic phrasing) selected when `candidateSwitchingBonus === 0`. [web/src/locales/en-US/translation.json], [web/src/locales/de-DE/translation.json], [web/src/components/tariff/tariff-comparison-form.tsx:247-249]
- [x] [Review][Patch] No test exercises the "both rows worth switching" verdict combination — Dev Notes explicitly require all three verdict combinations (both green, mixed, both red) to render correctly and warns "don't build to only the one worked example," but only the mixed and both-red combinations are exercised. Fixed — added a both-worth-switching test at both layers. [tests/EnergyTracker.Application.Tests/CompareTariffTests.cs], [web/src/components/tariff/tariff-comparison-form.test.tsx]
- [x] [Review][Defer] No automated check enforces the inline WCAG contrast-ratio claims in `index.css` [web/src/index.css] — deferred, pre-existing (the same gap exists for every other color-token pair in this file, e.g. the Status triad; not introduced by this story).
- [x] [Review][Defer] `compareTariff`'s response is cast via a bare `JSON.parse(text) as TariffComparisonDto` with no runtime validation [web/src/lib/tariff-api.ts:141] — deferred, pre-existing (identical unchecked-cast pattern used by every other function in this file).
- [x] [Review][Defer] No responsive/mobile layout verification for the new three-panel result layout [web/src/components/tariff/tariff-comparison-form.tsx] — deferred, pre-existing (applies broadly to UI stories in this codebase; uses the same established `GlassCard`/flex-col conventions already in use elsewhere).
...
None — no failing test/build cycles required debugging; implementation proceeded task-by-task per the story's own detailed guidance, verified incrementally against the mockup's worked numbers.

### Completion Notes List

- Task 1/2: Extended `CompareTariff.cs`/`TariffComparisonResult` and `TariffEndpoints.cs`/`TariffComparisonResponse` with `CandidateAnnualCostBonusIncluded`, `BonusIncludedAnnualSavings`, `IsBonusIncludedWorthSwitching`, `IsBonusNormalizedWorthSwitching` — exactly as specified, verdict computation inline in the Application layer (no new `Domain.Calculations` helper, per the story's explicit instruction). Verified against the mockup's worked example: bonus-included 836.80/337.20 (worth it), bonus-normalized unchanged at -12.80 (not worth it) — the two flags come out `true`/`false` respectively, confirming the bonus-included formula does NOT route through `BonusDecayNormalizer`.
- Task 3: Added the dedicated `--attractiveness-*` color tokens to `web/src/index.css`, dark-mode values verbatim from the mockup. Light-mode values had no mockup precedent (dark-only source file) and were independently derived (darker/more-saturated mint and clay-red than the dark pair, following the same approach the Status triad used for its own light/dark split) and **actually contrast-verified** with a WCAG relative-luminance script (not eyeballed) — all four AA-critical pairings clear 4.5:1 with margin: worth-it text 5.73:1, not-worth-it text 5.22:1, supporting-text 6.88-7.11:1 against both row tints, plus both badge fill/text pairs at 5.2-6.9:1. Unlike dark mode, light mode's raw `--attractiveness-not-worth-it` clears AA on its own as figure text (no separate lighter token needed there, though the token is still defined per-theme for structural consistency). The badge fill/text convention flips between themes (light pill/dark text in dark mode; dark pill/light text in light mode) because the raw tokens themselves had to become dark, saturated colors to work as light-surface figure text — documented inline in the CSS.
- Task 4/5: Replaced the single-line result block in `tariff-comparison-form.tsx` with current-vs-candidate summary panels (`SummaryRow`, mirroring `status-detail-dialog.tsx`'s label/value convention) and a two-way signal card (`SignalRow`, mirroring `status-card.tsx`'s verdict-driven class-map pattern via a simple ternary, since TS doesn't support `Record<boolean, string>` directly). Both rows always render together, colors/badges driven independently by each row's own server-computed verdict flag. No tariff names (data model has none). Switching-bonus summary row and bonus-included detail sentence both omitted when the bonus is `0`.
- Task 6: Added `tariff.compare.summary.*`/`signal.*` i18n keys to both locales, reusing existing `tariff.form.*`/`tariff.compare.candidate*Label` keys for the summary panels' base-fee/price/switching-bonus labels rather than adding near-duplicates. Removed the now-orphaned `savingsPositive`/`savingsNegative` keys from both locales after confirming (repo-wide grep) no other caller existed.
- Task 7: Extended `CompareTariffTests.cs`'s worked-example test with the four new-field assertions, plus three new cases (breakeven on each row independently, zero-switching-bonus equivalence). Extended `TariffEndpointsTests.cs`'s compare-200 test the same way. Rewrote `tariff-comparison-form.test.tsx` end-to-end (the old single-line assertions no longer apply) — 11 tests covering both-rows-together (AC #1), per-row breakeven (AC #2), summary-panel rendering and 2-decimal formatting (AC #4), the omitted-bonus-row case, and the single (not duplicated) low-confidence footnote.
- Verification: full backend suite green (600/600 across Application/Api/Infrastructure/Architecture.Tests, including Testcontainers Postgres+SqlServer), full frontend suite green (296/296), `dotnet build`/`tsc -b`/`oxlint`/`vite build` all clean. No regressions.

### File List

- `src/EnergyTracker.Application/CompareTariff.cs`
- `src/EnergyTracker.Api/Endpoints/TariffEndpoints.cs`
- `web/src/lib/tariff-api.ts`
- `web/src/index.css`
- `web/src/components/tariff/tariff-comparison-form.tsx`
- `web/src/locales/en-US/translation.json`
- `web/src/locales/de-DE/translation.json`
- `tests/EnergyTracker.Application.Tests/CompareTariffTests.cs`
- `tests/EnergyTracker.Api.Tests/TariffEndpointsTests.cs`
- `web/src/components/tariff/tariff-comparison-form.test.tsx`
...
- 2026-09-15: Story implementation complete (dev-story). All 7 tasks done: `CompareTariff`/`TariffComparisonResponse` extended with the bonus-included figure and both verdict flags; dedicated `--attractiveness-*` light/dark color tokens added with independently-derived, contrast-verified (WCAG relative-luminance, not eyeballed) light-mode values; current-vs-candidate summary panels and a two-way signal card built in `tariff-comparison-form.tsx`, replacing the old single-line result; i18n added to both locales, orphaned `savingsPositive`/`savingsNegative` keys removed. Full backend suite green (600/600), full frontend suite green (296/296), `dotnet build`/`tsc -b`/`oxlint`/`vite build` all clean. Status moves to review — no human sign-off gate reached in this pass.
