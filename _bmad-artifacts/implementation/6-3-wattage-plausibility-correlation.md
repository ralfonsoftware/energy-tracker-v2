...
5. **Given** the AI backend, **when** configured, **then** whether AI Wattage Plausibility is enabled is a Household-level setting, always visible and under the household's control — the choice of *which* backend (local model e.g. LMStudio, vs. cloud/external API) is one deployment-wide config value per AD-8, not a per-Household choice (FR-17, AD-8; wording corrected 2026-09-21 code review to match the accepted AD-8 trade-off — see deferred-work.md).
...
- [x] [Review][Defer] AC #5 backend choice is deployment-wide config, not literally Household-level [src/EnergyTracker.Api/Program.cs:346-363] — deferred, reason: accepted trade-off vs. AD-8 — deployment-wide backend selection is the correct read of AD-8's "one config value at composition root" rule; AC #5's wording will be corrected to match the code, not the other way around.
- [x] [Review][Patch] `OpenAiCompatibleClient` hardcodes `Model: "default"` with no config override — breaks the story's own named example cloud backend [src/EnergyTracker.Infrastructure/Adapters/OpenAiCompatibleClient.cs:47-50]. Fixed: added configurable `AiPlausibility:Model` (defaults to `"default"`) threaded through `AiPlausibilityBackendOptions`.
- [x] [Review][Patch] `WindowedDeviationCalculator` has no rollover/regression-correction awareness [src/EnergyTracker.Domain/Calculations/WindowedDeviationCalculator.cs:37-54]. Fixed: pairwise walk with the same resolved-Rollover/Reset correction `PatternDetectiveCalculator.ComputePaceToDate` uses; `CorrelateEvent` now feeds it `GetResolvedForMainMeterAsync`.
- [x] [Review][Patch] `OpenAiCompatibleClient.ClassifyAsync` doesn't catch `NotSupportedException`, violating its own documented "never throws" (NFR14) contract [src/EnergyTracker.Infrastructure/Adapters/OpenAiCompatibleClient.cs:69-87]. Fixed: added a catch clause; test added for the non-JSON-content-type path.
- [x] [Review][Patch] `CreateEvent.ExecuteAsync` doesn't guard `jobQueue.EnqueueAsync` — a transient enqueue failure surfaces as a failed create even though the Event was already persisted [src/EnergyTracker.Application/CreateEvent.cs:80-88]. Fixed: wrapped in try/catch with a warning log; create still succeeds.
- [x] [Review][Defer] Correlation computed exactly once, immediately after Event creation — for a real-time-logged Event the ±7-day window's forward half has no readings yet [src/EnergyTracker.Application/CorrelateEvent.cs:45-49] — deferred, reinforces the already-filed backfill-non-recompute item in deferred-work.md
- [x] [Review][Defer] `GET`/`PUT /households/{id}/ai-plausibility` call `SingleAsync` with no not-found guard [src/EnergyTracker.Api/Endpoints/HouseholdEndpoints.cs:145] — deferred, pre-existing pattern copied from the existing GET endpoint (line 97), unreachable today since Households are never deleted
- [x] [Review][Defer] AI classification round-trips add contention to the single-worker background job queue (AD-6) [src/EnergyTracker.Infrastructure/Adapters/OpenAiCompatibleClient.cs:20-24] — deferred, pre-existing AD-6 single-worker limitation this feature exercises more than prior features; feature is opt-in/default-off
...
- 2026-09-21: Code review completed. 3 decision-needed findings resolved with Ralf (AC #5 deferred — deployment-wide backend selection is the accepted read of AD-8, AC #5's wording corrected to match; Model config gap and rollover-correction gap both patched). 4 patch findings applied: `AiPlausibility:Model` is now configurable (was hardcoded `"default"`, which real OpenAI rejects); `WindowedDeviationCalculator` now corrects for a resolved Rollover/Reset landing inside its window (mirrors `PatternDetectiveCalculator.ComputePaceToDate`); `OpenAiCompatibleClient.ClassifyAsync` now catches `NotSupportedException` (non-JSON content-type) per its own never-throws contract; `CreateEvent.ExecuteAsync` no longer fails the create request when the `CorrelateEvent` enqueue itself fails. 4 findings deferred to `deferred-work.md` (AC #5 above, plus 3 pre-existing/accepted-risk items). 10 findings dismissed as noise/unreachable/already-covered. Full backend suite re-verified green (347 Application + 216 Infrastructure + 206 Api + 4 Architecture), `dotnet build` clean. Status set to done.
