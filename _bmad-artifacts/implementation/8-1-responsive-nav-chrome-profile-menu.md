...
- **AC #2** (bottom tab bar unchanged at <660px): `wide:hidden` on the existing bottom-tab-bar markup, otherwise byte-for-byte unchanged. Proven by `nav-chrome.test.tsx` (existing assertions retained, now scoped to the bottom variant) and `web/e2e/app-shell.spec.ts`'s new Playwright spec at a real 375×800 viewport (bottom bar visible, top nav hidden) — the manual live-Chrome half of this check was blocked by a sandbox window-resize limitation (Debug Log), substituted with this real-Chromium Playwright proof per Task 7's own guidance.
