...
- **Re-imported Smart Plug data can't silently duplicate *or* silently drift.** AD-20 already made re-imports duplicate-safe (a DB-level uniqueness constraint on Power Point + timestamp). AD-22 closes the other half of the same worry: if Eve Home or Meross ever revises a value it already reported for a timestamp already stored, that's now actually noticed — the watermark comparison catches it, and the fix runs through the same `AuditCorrection` mechanism corrections already use elsewhere, rather than a value silently changing with no trace. One real limit worth knowing: this only catches a revision on the *same* device/Power Point stream — it wouldn't notice a different physical plug quietly starting to report under an existing Power Point's name. That's a known, deliberately deferred gap, not something this closes.
- **Smart Plug imports move to one bulk-write path, not two.** AD-23 replaces the previous "try one big save, fall back to inserting rows one-by-one on conflict" dance with a single call to a purpose-built bulk-write library (`EFCore.BulkExtensions`), used the same way for every import regardless of size — a 20-row weekly delta and a 100,000+-row full-history Eve import go through the identical path, on both database providers. It's licensed a little differently from everything else in this stack — free at this project's personal scale, but worth knowing it isn't MIT like most dependencies here — and it comes with a condition: a throwaway-schema spike has to confirm real throughput and that a mid-write failure still can't leave a half-written import behind, before it's trusted at full scale. "Should be faster" isn't the same as measured.
