...
- §4.6 FR-33: Logoff performs full federated sign-out (RP-initiated logout at the OIDC provider, not just clearing the local app cookie), with graceful local-only fallback if the configured provider doesn't support `end_session_endpoint`. *Confirmed with Ralf.*
