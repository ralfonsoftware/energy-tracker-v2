...
**Mobile nav pattern:** a bottom tab bar carries the 3–4 most-used surfaces — Dashboard, Trend History, Tariff Radar, Settings — using `{components.nav-chrome}`'s active-state treatment; less-frequent surfaces (Onboarding/Household Setup, Log Event) are reached through Settings or contextual entry points rather than claiming their own tab, keeping the persistent chrome as quiet as the rest of the product. Smart Plug Import is a contextual entry point too, but as of the FR-4 amendment (2026-08-26) it is reached via a compact icon button on **both** Dashboard and Trend History — no longer nested inside Settings — rather than claiming a fifth tab; both icons open the same shared screen (upload + household-wide Job Status & History, FR-32). *(Amended 2026-09-23 — see Responsive & Platform below)* this pattern applies below 660px only; a top-nav variant with a Profile menu takes over above it.

## Responsive & Platform

*(Added 2026-09-23, Epic 8 — a 2026-09-23 UX review of all 9 production screens at desktop and tablet width found the wide-viewport rule below was already documented in intent but had never actually been applied to any shipped surface; the mobile single-column layout was rendering ungoverned across the full desktop/tablet window instead. This section is the corrected, authoritative statement.)*

**Breakpoint:** `{spacing.breakpoint-wide}` = 660px, one rule for both tablet and desktop widths — there is no separate third layout tier. Below it, every surface (Dashboard, Trend History, Tariff Radar, Settings) renders the single-column phone layout edge-to-edge, as already specified in Foundation/Information Architecture. At 660px and above, **every** surface — not only Trend History as originally scoped — constrains its content to a single 660px-wide column, centered in the remaining viewport width, still single-column-of-cards internally (no dense multi-column grid; that would work against the Status card's "one thing that matters most" discipline — see Do's and Don'ts in `DESIGN.md`).

**Nav chrome swap:** below 660px, the bottom tab bar (Foundation, above). At 660px and above, the identical four destinations render as a top navigation bar instead, using the same active/inactive tokens — a breakpoint-driven variant of one component, not a second one to maintain (`DESIGN.md` → Components → Nav chrome).

**Profile menu (new, ≥660px only):** an avatar button at the top-nav's right edge opens a dropdown with the account email, a "Profile" entry, and a "Log off" entry. "Log off" triggers the identical flow FR-33/Story 1.12 already built (pending-offline-reading check, federated-logout warning where applicable, `/logout` navigation) — this is a second reachable surface for that existing, already-`done` control, not a new implementation. Story 1.12's original control on the Settings page is unchanged and remains the only Logoff entry point below 660px.

**Card-hierarchy discipline:** the glass-vs-quiet rule (token definitions in `DESIGN.md` → Colors) now applies at ≥660px to: Trend History's Room → Power Point → Device card (quiet — reference only) next to its Meter Readings list (glass — has an Edit action); Tariff Radar's "current tariff"/"candidate tariff" summary panels (quiet) next to its attractiveness-signal verdict card (glass — the actual payoff, not passive reference).

**Settings restructuring (≥660px):** content is grouped into labeled sections — "Yearly Baseline", "AI Plausibility Check", "Household" (containing "Invite a member" as a normal row, alongside data export/import), "Rooms, Power Points & Devices" — rather than a flat, ungrouped stack of identically-weighted cards. "Invite a member" specifically moves into the "Household" section; it no longer renders as a standalone unheaded button below the Room tree. The Room → Power Point tree replaces equally-weighted bordered-button rows at every depth with a chevron + item-count summary row at the Room/Power-Point level (e.g. "Living Room — 5 Power Points"); "Add Power Point"/"Add Device" render as a text link at the end of the expanded level rather than a persistently-visible filled pill.

→ See [mockups/critique-desktop-breakpoint-2026-09-23.html](mockups/critique-desktop-breakpoint-2026-09-23.html) for the full before/after per surface (Dashboard, Trend History, Tariff Radar × 2 forms, Settings × 3 sections) that this section summarizes, including the Profile menu shown open. The existing `key-*.html` mocks referenced elsewhere in this document predate Epic 8 and still show the pre-correction mobile-only layout and bottom-tab-bar-only nav chrome; promoting the corrected desktop/tablet state into each of them is a follow-up, not yet done as of this update.
...
| Settings | Global nav | Yearly Baseline (FR-2), trending threshold, Tariff Check cadence, AI backend choice for Wattage Plausibility (local vs. cloud), Room/Power Point/Device management (FR-28), data export/import (FR-22/23), member invitation (FR-27). *(Amended 2026-09-23)* Grouped into labeled sections at ≥660px — see Responsive & Platform. |
...
| Nav chrome | Global (bottom tab bar <660px, top nav bar ≥660px — see Responsive & Platform) | Active tab/link uses `{components.nav-chrome.active-bg}` / `-dark` and `{components.nav-chrome.active-foreground}` / `-dark` — brand-accent-tinted, never a status color. Reflects the current top-level surface only (Dashboard, Trend History, Tariff Radar, Settings); no badge/counter treatment. |
| Profile menu | Global, top-nav variant only (≥660px — see Responsive & Platform) | Avatar button opens a dropdown: account email, "Profile", "Log off" (reuses Story 1.12/FR-33's existing logoff flow). Added 2026-09-23, Epic 8. |
...
| Unit-inside-field (kWh/EUR entry) | Yearly Baseline, Log Reading sheet, **Tariff Radar's Add-Tariff and Compare-Tariff forms (added 2026-09-23, Epic 8, UX-DR23)** | Any bare-number-plus-unit entry renders the unit suffix *inside* the same bordered field as the number, never as a separate label beside it — one composed field, so the unit can't wrap onto its own line independently of the value. The Log Reading sheet's kWh field originated this pattern; Yearly Baseline's field was aligned to match it, and Tariff Radar's Grundgebühr/Preis-pro-kWh/Wechselbonus fields were aligned to match it in Epic 8. At the ≥660px breakpoint, Tariff Radar additionally pairs two related fields per row (e.g. Grundgebühr + Preis/kWh) instead of stacking each field alone on full width. |
