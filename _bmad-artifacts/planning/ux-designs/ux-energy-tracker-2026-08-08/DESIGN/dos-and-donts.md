...
| *(added 2026-09-23, Epic 8)* Use `{colors.surface-quiet}` / `-dark` for any card across the product that is a pure read-only reference display with no interactive control (e.g. Tariff Radar's current/candidate summary panels, Trend History's Room → Power Point → Device card) | Give a reference-only card the same `{colors.surface-glass}` blur/border treatment as an actionable card (one with an input, toggle, tree row, or button) — the two tiers exist specifically so a glance tells them apart |
